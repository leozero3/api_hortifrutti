...
export default class CreatePedidoValidator {
  ...
  public schema = schema.create({
    estabelecimento_id: schema.number([rules.exists({ table: 'estabelecimentos', column: 'id' })]),
    meios_pagamento_id: schema.number([rules.exists({ table: 'meios_pagamentos', column: 'id' })]),
    ...
    produtos: schema.array([rules.minLength(1)]).members(
      schema.object().members({
        produto_id: schema.number([rules.exists({ table: 'produtos', column: 'id' })]),
